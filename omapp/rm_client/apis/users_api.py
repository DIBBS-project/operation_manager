...
class UsersApi(object):
    ...
    def rsa_public_key_id_get(self, id, **kwargs):
        """
        Get the RSA public key of a user.
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.rsa_public_key_id_get(id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.rsa_public_key_id_get_with_http_info(id, **kwargs)
        else:
            (data) = self.rsa_public_key_id_get_with_http_info(id, **kwargs)
            return data

    def rsa_public_key_id_get_with_http_info(self, id, **kwargs):
        """
        Get the RSA public key of a user.
        Get the RSA public key of a user. Authentification is not required. 
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.rsa_public_key_id_get_with_http_info(id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method rsa_public_key_id_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `rsa_public_key_id_get`")
